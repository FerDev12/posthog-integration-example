CREATE TYPE "public"."quiz_categories" AS ENUM('JavaScript', 'TypeScript', 'React', 'Node', 'HTML', 'CSS', 'Go', 'Git', 'Frontend', 'Backend', 'Other');--> statement-breakpoint
...
CREATE TABLE "quizes" (
	...
	"description" text,
	"created_by" text,
	"category" "quiz_categories" NOT NULL,
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp with time zone DEFAULT now() NOT NULL
);
--> statement-breakpoint
...
ALTER TABLE "quizes" ADD CONSTRAINT "quizes_created_by_users_id_fk" FOREIGN KEY ("created_by") REFERENCES "public"."users"("id") ON DELETE set null ON UPDATE no action;